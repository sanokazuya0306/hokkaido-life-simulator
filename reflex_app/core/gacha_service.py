...
class GachaService:
    ...
    # ガチャ確率（10,000サンプル実測値、新配分: 寿命40%、生涯年収35%、学歴25%）
    ...
    def _generate_life_story(self, life: Dict[str, Any]) -> str:
        ...
        if life.get('university'):
            ...
            if uni_name:
                ...
                # 大学名に既に都道府県名が含まれている場合は prefix を付けない
                # 例: "奈良県の大学" という大学名の場合、"奈良県の奈良県の大学" を防ぐ
                if uni_dest and uni_dest != '北海道' and not uni_name.startswith(uni_dest):
                    lines.append(f"{uni_dest}の{uni_name}に進学")
                else:
                    lines.append(f"{uni_name}に進学")
            else:
                lines.append("大学に進学")
        ...
        went_to_university = life.get('university', False)
        went_to_high_school = life.get('high_school', False)
        went_to_graduate_school = life.get('graduate_school', False)
        
        if first_industry and company_size:
            # 大学院卒の場合
            if went_to_graduate_school or '大学院' in education_level:
                lines.append(f"大学院修了後、{first_industry}の{company_size}に{employment_type}として就職")
            # 大学卒の場合（universityフラグを優先チェック）
            elif went_to_university:
                lines.append(f"大学卒業後、{first_industry}の{company_size}に{employment_type}として就職")
            # education_levelで大学卒を確認（念のため）
            elif '大学' in education_level or '大学卒' in education_level:
                lines.append(f"大学卒業後、{first_industry}の{company_size}に{employment_type}として就職")
            # 高校卒の場合
            elif went_to_high_school:
                lines.append(f"高校卒業後、{first_industry}の{company_size}に{employment_type}として就職")
            # education_levelで高校卒を確認（念のため）
            elif '高校' in education_level or '高校卒' in education_level:
                lines.append(f"高校卒業後、{first_industry}の{company_size}に{employment_type}として就職")
            # その他
            else:
